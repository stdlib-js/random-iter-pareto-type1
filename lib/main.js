...
function iterator( alpha, beta, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0qH71', alpha ) );
	}
	if ( !isPositiveNumber( beta ) ) {
		throw new TypeError( format( '0qH72', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0qH2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qH2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
