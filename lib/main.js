...
function iterator( alpha, beta, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0Q27P', alpha ) );
	}
	if ( !isPositiveNumber( beta ) ) {
		throw new TypeError( format( '0Q27T', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Q22h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Q235', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
